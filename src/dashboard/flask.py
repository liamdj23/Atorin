...
class Dashboard(Flask):
    def __init__(self, bot):
        ...
        self.add_url_rule(
            "/server/<server_id>/<setting>",
            view_func=panel.server,
            methods=["GET", "POST"],
        )
...
class Discord:
    ...
    def get_user(self, access_token):
        if access_token:
            r = requests.get(
                self.me_url, headers={"Authorization": "Bearer {}".format(access_token)}
            )
            r.raise_for_status()
            return r.json()
        return
    ...
    def get_guilds(self, access_token):
        r = requests.get(
            self.guilds_url, headers={"Authorization": "Bearer {}".format(access_token)}
        )
        r.raise_for_status()
        return r.json()
    ...
    def get_authorize_url(self):
        return (
            "https://discord.com/oauth2/authorize?client_id={}&redirect_uri={}"
            "/login&response_type=code&scope=identify%20guilds".format(
                self.config["discord_oauth2_id"], self.config["discord_oauth2_domain"]
            )
        )
