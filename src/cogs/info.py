...
class Info(commands.Cog, name="ℹ Informacje"):
    ...
    async def invite(self, ctx):
        await ctx.send(
            "🔹 Dodaj Atorina na swój serwer, korzystając z tego linku: https://liamdj23.ovh/addbot\n"
    ...
    async def server(self, ctx):
        ...
        embed.add_field(
            name="👶 Data utworzenia", value=guild.created_at.replace(microsecond=0)
        )
    ...
    async def user(self, ctx, member: discord.Member = None):
        ...
        embed.add_field(
            name="🤝 Data dołączenia", value=member.joined_at.replace(microsecond=0)
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if r.status == 200:
                    ...
                    embed.add_field(
                        name=emoji + " Pogoda", value=data["weather"][0]["description"]
                    )
                    embed.add_field(
                        name="🌡️ Temperatura",
                        value=str(data["main"]["temp"]) + "°C",
                        inline=False,
                    )
                    embed.add_field(
                        name="🎈 Ciśnienie", value=str(data["main"]["pressure"]) + "hPa"
                    )
                    embed.add_field(
                        name="💧 Wilgotność", value=str(data["main"]["humidity"]) + "%"
                    )
                    embed.add_field(
                        name="💨 Wiatr", value=str(data["wind"]["speed"]) + "m/s"
                    )
                    await ctx.send(embed=embed)
                elif r.status == 404:
    ...
    @commands.command(
        aliases=["pomoc", "komendy", "?"], description="Lista komend AtorinBot"
    )
        ...
        if not arg:
            ...
            for name, cog in self.bot.cogs.items():
                if name != "StatcordPost":
                    ...
                    embed.add_field(
                        name=name, value="```{}```".format(cog_commands), inline=False
                    )
            await ctx.send(embed=embed)
            ...
            if cmd.usage:
                embed.add_field(
                    name="▶ Komenda",
                    value="```{} {}```".format(cmd.name, cmd.usage),
                    inline=False,
                )
            else:
                embed.add_field(
                    name="▶ Komenda", value="```{}```".format(cmd.name), inline=False
                )
            if cmd.aliases:
                embed.add_field(
                    name="🔠 Aliasy",
                    value="```{}```".format(", ".join(cmd.aliases)),
                    inline=False,
                )
    ...
    @commands.command(
        description="Kup lub sprawdź status Atorin Premium", aliases=["donate"]
    )
    async def premium(self, ctx):
        ...
        if data:
            embed.description = (
                "💎 {}, Twoje premium jest **aktywne** i jest ważne do **{}**".format(
                    ctx.author.mention, data.expire.strftime("%d/%m/%Y %H:%M")
                )
            )
        else:
            embed.description = (
                "💎 Wesprzyj bota kupując Atorin Premium, w zamian otrzymasz dostęp do eksluzywnych"
