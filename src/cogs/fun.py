...
class Fun(commands.Cog, name="🎲 Zabawa"):
    ...
    @commands.command(
        description="Wpisz aby otrzymać losowe zdjęcie kotka", aliases=["kot"]
    )
    ...
    @commands.command(
        description="Wpisz aby otrzymać losowe zdjęcie lisa", aliases=["lis"]
    )
    ...
    @commands.command(
        usage="<nazwa>", description="Pokazuje profil z Instagrama", aliases=["ig"]
    )
    async def instagram(self, ctx, name: str):
        ...
        r = requests.get(
            "https://www.instagram.com/{}/?__a=1".format(name.replace("@", "")),
        ...
        embed.add_field(
            name="🔗 Link",
            value="https://instagram.com/{}".format(user["username"]),
            inline=False,
        )
    ...
    @commands.command(
        description="Wpisz aby otrzymać losowe zdjęcie pieska",
        aliases=["pies", "piesek"],
    )
    ...
    async def flip(self, ctx):
        ...
        embed.description = "🪙 **{}**".format(
            "Orzeł" if randrange(2) == 1 else "Reszka"
        )
        await ctx.send(embed=embed)

    @commands.command(
        description="Generuje mema Change My Mind", usage="<tekst>", aliases=["cmm"]
    )
    async def changemymind(self, ctx, *, text: str):
        if len(text) > 140:
            await ctx.send(
                "❌ Zbyt duża ilość znaków! (Wprowadzono {}, max. {})".format(
                    len(text), 140
                )
            )
            return
    ...
    @commands.command(
        description="Wysyła losowego mema z /r/Polska_wpz", aliases=["mem", "memy"]
    )
    ...
    @commands.command(
        description="Wysyła losowe zdjęcie żółwia", aliases=["zółw", "zolw"]
    )
    ...
    @commands.command(
        description="Wysyła losowe zdjęcie żaby", aliases=["żaba", "zaba"]
    )
